...
use crate::applications::{AppState, ApplicationsData};
...
fn get_latest_data_id(conn: &Connection) -> Result<i64, String> {
    let mut stmt = conn.prepare("SELECT MAX(_id) FROM DATA")
        .map_err(|e| format!("Failed to prepare query for latest ID: {}", e))?;
    
    let latest_id: i64 = stmt.query_row([], |row| {
        row.get(0)
...
pub fn backup_sqlite_on_startup() -> Result<(), String> {
    ...
    if !db_path.exists() {
        println!("Settings database not found at: {}, skipping backup", db_path.display());
        return Ok(());
    }
...
pub fn load_and_store_sqlite_data(app_handle: &AppHandle) -> Result<ApplicationsData, String> {
    ...
    if !db_path.exists() {
        return Err(format!("Settings database not found at: {}", db_path.display()));
    }
    ...
    let mut stmt = conn.prepare("SELECT _id, FILE FROM DATA WHERE _id = ?")
        .map_err(|e| format!("Failed to prepare blob query: {}", e))?;
    
    let blob_data: Vec<u8> = stmt.query_row([latest_id], |row| {
        row.get::<_, Vec<u8>>(1)
    ...
    let mut db_data = state.settings_db_data.lock()
    ...
    *apps = applications_data.applications.clone();
    ...
    println!("Successfully loaded {} applications from SQLite database", applications_data.applications.len());
...
pub fn write_to_sqlite(app_handle: &AppHandle) -> Result<(), String> {
    ...
    if !db_path.exists() {
        return Err(format!("Settings database not found at: {}. Cannot write to non-existent database.", db_path.display()));
    }
    ...
    println!("Successfully saved {} applications to SQLite database with ID: {}", apps.len(), new_id);
    Ok(())
}
